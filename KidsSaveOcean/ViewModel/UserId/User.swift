...
class User: Codable {
    ...
    static func fetchUserFBData() {
        ...
        databaseRef?.observeSingleEvent(of: .value) {  snapshot in
            ...
            guard let dictionary = snapshot.value as? Dictionary<String, Any>,
                let userViewModel = User(with: dictionary) else {
                return
            }
    ...
    func save() {
        ...
        User.databaseRef?.setValue(userModelDictionary) { (error: Error?, _: DatabaseReference) in
    ...
    func isUserLocationCampaignIsLive() -> Bool {
        let campaignLive = CampaignViewModel.shared.campaigns.filter({$0.id == campaign?.id}).first?.live
